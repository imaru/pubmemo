...
## 4. 一般線形モデルの例: 説明変数が無く、正規分布を仮定するモデル
  * ビール売り上げの例
    * 凡例
      * $y_i$:売り上げデータ
      * $\mu_i$: $y_i$の期待値
      * $_i$: 何番目のデータか
      * $g$: 恒等関数（$g(y)=y$となる関数=何も怒らない関数）
    * 一般性系モデル
      * $g(\mu_i)=\beta_0$
      ...
      * ここから、ちょっと馬場本の式3.10, 3.11が分かりにくく感じたので、緑本p159の説明順を参考に表現を変えます。なんでexp()なのか、logなのかの説明がなくて分からない。
        ...
        * ポアソン回帰には対数リンク関数、ロジスティック回帰にはロジットリンク関数＝正準リンク関数
          ...
          * なぜポアソン回帰には対数リンク関数を使うのか？
            ...
            2. わかりやすいから（緑本P59あたり）：効果が掛け算になる。$exp(\beta_0+\beta_1x_1)$は$exp(\beta_0)\times exp(\beta_1x_!)$。複数の効果が掛け算で効いてくる方がもっともらしい。
